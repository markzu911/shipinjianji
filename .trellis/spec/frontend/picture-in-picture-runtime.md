...
修改画中画素材注册表、顶层 inspector、生成/轮询 effect、公共预览/时间线、compose 投影或项目草稿时适用。默认主编辑器必须使用顶层 `PipTool`；B3 期间旧 iframe 和独立页只作为 feature flag fallback，B4 才删除。
...
- 画中画宽度必须 finite 且 `>= EditorPipModel.MIN_WIDTH`（当前 15%），不设最大值。面板 number input 不含 `max`；公共 compositor、fallback、Store、草稿和后端共用相同最小值与中心裁切语义。
...
- 默认 `window.__EDITOR_PIP_PANEL_ENABLED__ !== false` 时不得创建 pip iframe；设为 false 并 reload 后，只运行 legacy iframe authority。B3 保留 `/picture-in-picture`、`embedded=1` 和消息桥，禁止默认与 fallback 同时写 Store。
...
- Base：schema v1 只恢复艺术字；pip 继续使用当前服务端素材和 overlay。feature flag false 时旧 iframe 仍可编辑并回写 Store。
...
- 浏览器：prompt/image/video 全部 mock；覆盖 completed/failed、enable/disable、selection、position、range、175%、v2/v1 reload、无效 selection、迟到响应、fallback、standalone 和 375px 无溢出。
