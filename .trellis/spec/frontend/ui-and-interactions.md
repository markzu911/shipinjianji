...
- 播放头“分割”按钮位于剪辑时间轴标题右侧且点击区不少于 `44px`；起点、终点、已有边界、删除区和不足最小时长时原生 disabled。split clip/恢复 marker 必须阻止自由拖选 handler，键盘 selection、Delete/Backspace、撤销/重做和重绘后焦点都按稳定 clip key 工作。
- 删除完整分割片段沿用确认弹窗，但提示必须明确边界不会被语音保护移动；全片删除后时间轴仍显示可聚焦恢复 marker。多个 marker 映射到同一拼接点时向内分层，不能溢出左右边缘或用无效的 CSS 乘法表达式。
