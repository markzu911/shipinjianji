...
- 展示片段编辑：用一个父段内的“删除/保留/删除”和重复短语 fixture，同时断言 run Unicode code point 范围、点击行与弹窗文字/剪后时间、局部 text/split/merge payload、过期 run 拒绝、非法方向 400、被隔离删除文字可恢复，以及 Store/art/公共时间轴/preview/compose 字符守恒。必须检查基础 video `src/load()` 和 extractor 增量为 0；禁止用 `indexOf()` 或只测整段掩盖重复实例错误。
