...
需要 Python 3.11/3.12、FFmpeg 和一个在线语音识别 API Key。为避免删除文案后残留尾音，应用在首次需要校准语音剪辑边界时会按需下载固定版本的 FunASR `fa-zh`（约 159 MB）和 FSMN-VAD `fsmn-vad`（约 1.7 MB）本地模型到 `DATA_DIR/models`。ASR/强制对齐确认相邻字符，VAD 与 PCM 共同确认字符间无声区；任一模型下载、校验、加载或推理失败时会安全降级，不阻断文案拆分、草稿保存或生成。

模型下载、校验、加载或推理失败时，剪辑草稿会安全降级到受保护的既有边界并记录诊断，不会因此无法保存；发布或离线使用前仍应提前完成一次模型下载和真实语音剪辑验证。
