...
需要 Python 3.11/3.12、FFmpeg 和一个在线语音识别 API Key。为避免删除文案后残留尾音，应用在首次需要校准语音剪辑边界时会按需下载固定版本的 FunASR `fa-zh` 本地模型到 `DATA_DIR/models`；模型权重约 159 MB，Python 依赖和运行内存还会额外占用空间。
